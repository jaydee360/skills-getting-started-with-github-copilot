...
def signup_for_activity(activity_name: str, email: str):
    ...
    print(f"Attempting to sign up {email} for {activity_name}")
    if email in activity["participants"]:
        raise HTTPException(status_code=400, detail="Student already signed up for this activity")

    # Add student
    activity["participants"].append(email)
    print(f"Current participants in {activity_name}: {activity['participants']}")
...
def unregister_participant(activity_name: str, email: str):
    ...
    print(f"Unregistered {email} from {activity_name}. Current participants: {activity['participants']}")
